...
import { guessBrandAndName, matchProductsFromText, type PhotoMatch } from '@/engine/photoMatch';
...
/**
 * 사진으로 제품 찾기 / 등록.
 * 1) 사진 선택 → 2) 기기 안에서 OCR → 3) 읽어낸 글자로 제품 대조 → 담기, 또는 4) 직접 등록.
 */
export function PhotoSearchOverlay({ registerOnly = false }: Props) {
  ...
  const matches: PhotoMatch[] = useMemo(() => (text.trim() ? matchProductsFromText(text, allProducts) : []), [text, allProducts]);
  const guess = useMemo(() => guessBrandAndName(text, allProducts), [text, allProducts]);

  const onFile = async (file: File | undefined) => {
    if (!file) return;
    setError(null);
    try {
      const [big, small] = await Promise.all([resizeImage(file, 1400, 0.9), resizeImage(file, 420, 0.8)]);
      setPreview(big);
      setThumb(small);
    ...
    <Overlay title={registerOnly ? '내 제품 직접 등록' : '사진으로 제품 찾기'} onClose={popOverlay}>
      {stage === 'pick' && (
        <div className="stack">
          <p className="small muted">
            제품 앞면(브랜드·제품명이 보이는 면)을 찍어주세요. 사진은 서버로 보내지 않고 이 기기 안에서 글자만 읽어내요.
          </p>
      ...
      {stage === 'recognizing' && (
        <div className="stack">
          ...
          <div className="card card--soft">
            <div className="h3">{ocrStatusKo(progress.status)}</div>
            <div className="progress mt-2">
              <div className="progress__bar" style={{ width: `${Math.round(progress.progress * 100)}%` }} />
            </div>
            <p className="tiny muted mt-1">
              {progress.status.includes('recognizing') ? `${Math.round(progress.progress * 100)}%` : '처음 실행 시 언어 데이터를 내려받아 시간이 걸릴 수 있어요.'}
            </p>
          </div>
        </div>
      )}
      ...
      {stage === 'results' && (
        <div className="stack">
          ...
          <textarea className="textarea" style={{ minHeight: 70 }} value={text} onChange={(e) => setText(e.target.value.slice(0, 400))} placeholder="읽어낸 글자가 없어요. 브랜드나 제품명을 직접 입력해 대조해보세요." />
          {error && <div className="notice notice--danger">{error}</div>}

          <div className="row row--between">
            <span className="h3">일치하는 제품 {matches.length}개</span>
            <Badge>{activeRoutine === 'AM' ? '아침' : '저녁'} 루틴에 담기</Badge>
          </div>
          {matches.length === 0 ? (
            <div className="empty">
              <strong>일치하는 제품을 찾지 못했어요</strong>
              글자를 고쳐 다시 대조하거나, 아래에서 직접 등록해주세요.
            </div>
          ) : (
      ...
      {stage === 'register' && (
        <CustomProductForm
          initialBrand={guess.brand}
          initialName={guess.name}
          imageUrl={thumb}
